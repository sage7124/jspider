...
router.put('/slots/:userId', async (req, res) => {
    try {
        ...
        for (const ext of existingSlots) {
            const isStillActive = slots.some(s => s.dayOfWeek === ext.dayOfWeek &&
                s.slotNo === ext.slotNo &&
        ...
        for (const s of toCreate) {
            const alreadyExists = existingSlots.some(ext => ext.dayOfWeek === s.dayOfWeek &&
                ext.slotNo === s.slotNo &&
...
function parseCollegeVisit(b) {
    ...
    const formatLocationWithMapLink = (locName, lat, lng) => {
        if (locName && lat && lng) {
                        ...
                        if (type === 'COLLEGE_VISIT') {
                            ...
                            if (dayBreaks.length === 1) {
                                ...
                                if (firstB.punchInLat && firstB.punchInLng) {
                                    ...
                                    punchInLocVal = { text: firstB.punchInLocation ? `${firstB.punchInLocation} (View Map)` : 'View Map', hyperlink: inMapUrl };
                                }
                                else {
                                ...
                                if (firstB.punchOutLat && firstB.punchOutLng) {
                                    ...
                                    punchOutLocVal = { text: firstB.punchOutLocation ? `${firstB.punchOutLocation} (View Map)` : 'View Map', hyperlink: outMapUrl };
                                }
                                else {
