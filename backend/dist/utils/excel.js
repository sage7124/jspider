...
const getTraineeReportData = (user, attendances, year, mon, daysInMonth, holidays = [], leaves = [], earlyLeaves = []) => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (cvLog) {
            ...
            if (att.inTime)
                cvInStr = new Date(att.inTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
            if (att.outTime)
                cvOutStr = new Date(att.outTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
