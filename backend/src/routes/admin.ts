...
const calcBreakDurationMins = (breakOut: Date | string, breakIn: Date | string): number => {
      ...
      for (const user of targetUsersList) {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
          ...
          if (dayBreaks.length > 0) {
            ...
            const punchInLocList: string[] = [];
            const punchOutLocList: string[] = [];
...
router.get('/other-center-classes', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const { status, search, month } = req.query;
...
router.post('/other-center-classes/process', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const { logId, status, adminReason } = req.body; // status: APPROVED or REJECTED
...
router.get('/reports/other-center-classes/export', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const { month, search } = req.query;
    if (!month || typeof month !== 'string') {
      return res.status(400).json({ error: 'Month is required' });
...
router.post('/other-center-classes/log', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const { traineeId, date, subject, batchNo, duration, startTime, endTime, noOfStudents, centerName, remarks, classMode } = req.body;
...
router.put('/other-center-classes/:id', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    const { traineeId, date, subject, batchNo, duration, startTime, endTime, noOfStudents, centerName, remarks, classMode } = req.body;
