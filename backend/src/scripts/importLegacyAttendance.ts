...
async function runMigration() {
  ...
  const dbA = new PrismaClient({ datasources: { db: { url: legacyUrlA } } });
  const dbB = new PrismaClient({ datasources: { db: { url: legacyUrlB } } });
  ...
  try {
    ...
    const targetUserMap = new Map<string, number>();
    targetUsers.forEach(u => {
      if (u.identifier) targetUserMap.set(u.identifier.trim(), u.id);
    });

    const userMapA = new Map<number, string>();
    usersA.forEach(u => {
      if (u.identifier) userMapA.set(u.id, u.identifier.trim());
    });

    const userMapB = new Map<number, string>();
    usersB.forEach(u => {
      if (u.identifier) userMapB.set(u.id, u.identifier.trim());
    });
