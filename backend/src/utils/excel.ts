...
export const getTraineeReportData = (user: any, attendances: any[], year: number, mon: number, daysInMonth: number, holidays: any[] = [], leaves: any[] = [], earlyLeaves: any[] = []) => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const daySlots = user.slots?.filter((s: any) => {
      ...
      return true;
    }).sort((a: any, b: any) => a.slotNo - b.slotNo) || [];
    ...
    const calcEarly = (slot: any, dayOutTime?: Date, dayInTime?: Date, slotOutTime?: Date, slotInTime?: Date, isExtra?: boolean) => {
      ...
      if (inTime) {
        ...
        if (normIn.getTime() > slotEnd.getTime()) return '--';
      }
      if (!outTime) {
...
export const generateTraineeWorksheet = (ws: exceljs.Worksheet, user: any, attendances: any[], year: number, mon: number, daysInMonth: number, holidays: any[] = [], leaves: any[] = [], earlyLeaves: any[] = []) => {
  ...
  const hasCollegeVisits = (user.breakLogs || []).some((b: any) => b.collegeName || (b.reason && b.reason.startsWith('College Visit:')));
